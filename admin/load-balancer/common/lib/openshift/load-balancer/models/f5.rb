...
module OpenShift
  ...
  class F5LoadBalancerModel < LoadBalancerModel
    ...
    def create_monitor monitor_name, path, up_code, type
      ...
      @bigip['LocalLB.Monitor'].create_template [{:template_name=>monitor_name, :template_type=>'TTYPE_HTTP'}], [{:parent_template=>type, :interval=>30, :timeout=>5, :dest_ipport=>{:address_type=>'ATYPE_STAR_ADDRESS_STAR_PORT', :ipport=>{:address=>'0.0.0.0', :port=>0}}, :is_read_only=>false, :is_directly_usable=>true}]
